...
public class ExponentialHistogramCircularIntBackup implements slidingwindow {
	final int k;
	final int halfk;
	...
	public ExponentialHistogramCircularIntBackup clone() {	
		return new ExponentialHistogramCircularIntBackup(this);
	}
	...
	static void testSingle() {
		...
		for (float epsilon=0.025f;epsilon<0.21;epsilon+=0.025) {
			for (int repeat=0;repeat<repeats;repeat++) {
				...
				ExponentialHistogramCircularIntBackup dw = new ExponentialHistogramCircularIntBackup(epsilon, windowSize, maxNumberOfEvents);
...
//					}
	...
	public static void mainddd(String[]args) {
		...
		int queryLength=10;
		double epsilon=0.5;
		ExponentialHistogramCircularIntBackup ex = new ExponentialHistogramCircularIntBackup(epsilon, queryLength, 1000);
...
//		ex.getEstimationRealtimeWithExpiration(10000-100);
	...
	public static ExponentialHistogramCircularIntBackup mergeEHs(ExponentialHistogramCircularIntBackup[] ehs, double epsilon, int windowSize) {
		// now merge
		int totalEvents=0; int totalInterrupts=0;
		for (ExponentialHistogramCircularIntBackup eh:ehs) { 
		...
		for (int cnt=0;cnt<numberOfEH;cnt++) {
			ExponentialHistogramCircularIntBackup eh = ehs[cnt];
		...
		ExponentialHistogramCircularIntBackup ehAggregated=new ExponentialHistogramCircularIntBackup(epsilon, windowSize, totalEvents);
		// and now replay
	...
	public ExponentialHistogramCircularIntBackup(ExponentialHistogramCircularIntBackup eh) {
...
//		this.expirations = (TreeMap<Integer, LinkedList<Integer>>)eh.expirations.clone();
	...
	public ExponentialHistogramCircularIntBackup(double epsilon, int windowSize,  long maxEvents) {
