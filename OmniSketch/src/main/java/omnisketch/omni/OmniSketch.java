...
import omnisketch.Synopsis;
import omnisketch.Main;
import omnisketch.Query;
import omnisketch.Record;
...
//        long[] recordArray = record.getRecord();
//
//        for (int i = 0; i < org.Main.numAttributes; i++) {
//            if (hasPredicate[i]) {
...
//        //System.out.println("Add time: " + (end - start));
    ...
    private long[][] wrapperInitLogRanges(long l) {
        //l += (long) Math.pow(2, org.Main.dyadicRangeBits - 1); // shift to positive
        if (l < 0) {
        ...
        for (int i = 0; i < ranges[2].length; i++) {
            ranges[1][i] = ranges[1][i] - 1L ;//- (long) Math.pow(2, org.Main.dyadicRangeBits - 1);
            ranges[2][i] = ranges[2][i] - 1L ;//- (long) Math.pow(2, org.Main.dyadicRangeBits - 1);
        }
        return ranges;
    }
    ...
    public int query(Query q) {
        ...
        //        " org.Main.depth: " + org.Main.depth + " org.Main.maxSize: " + org.Main.maxSize + " org.Main.delta: " + org.Main.delta +
         //       " org.Main.eps: " + org.Main.eps);
        ...
        if (S_cap < constraint) {
            //return (int) Math.ceil(S_cap * n_max / org.Main.maxSize);
            //return (int) Math.ceil(S_cap * n_max / org.Main.maxSize);
    ...
    private ArrayList<long[]> wrapLogRanges(long low, long up) {
        ArrayList<long[]> temp;
        //low += (long) Math.pow(2, org.Main.dyadicRangeBits - 1);
        //up += (long) Math.pow(2, org.Main.dyadicRangeBits - 1);
        if (low < 0 || up < 0) {
        ...
        for (long[] i: temp) {
            i[0] = i[0] - 1L;// - (long) Math.pow(2, org.Main.dyadicRangeBits - 1);
            i[1] = i[1] - 1L;// - (long) Math.pow(2, org.Main.dyadicRangeBits - 1);
        }
        return temp;
    }
                ...
                if (Main.rangeQueries) {
                    for (int j = 0; j < Main.dyadicRangeBits + 1; j++) {
                        if (maxBits[i] + 1 < Main.dyadicRangeBits - j) { // no need to store empty ranges
                            ...
                            //System.out.println("No need to store empty range " + (org.Main.dyadicRangeBits - j) + " for attribute " + i);
                            continue;
                        }
