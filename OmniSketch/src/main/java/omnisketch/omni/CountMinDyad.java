...
/*
    public int[] testAdd(int id, int attr) {
        return hash(attr, org.Main.depth, org.Main.width);
    }

    public void testUniformity() {
        System.out.println("Testing Uniformity");
        int[] counts = new int[org.Main.width];
        for (int i = 0; i < org.Main.width; i++) {
            counts[i] = 0;
        }
        for (int i = 0; i < 100000; i++) {
            int[] hashes = hash(i, org.Main.depth, org.Main.width);
            for (int j = 0; j < org.Main.depth; j++) {
                int w = hashes[j];
                counts[w]++;
            }
        }
