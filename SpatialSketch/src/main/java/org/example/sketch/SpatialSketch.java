...
//            for (int j = 0; j < sketches[0].length; j++) {
//                if (synopsisID == 1) {
//                    sketches[i][j] = new CountMin(uid, basicSketchParametersArray);
//                } else {
//                    throw new IllegalArgumentException("Invalid synopsis ID for SpatialSketch: " + synopsisID + " (expected 1)");
//                }
//            }
//        }
    ...
    private int getGridIndex(int coord, int minDomain, int maxDomain,int res) {
        return (int) Math.floor((double) (coord - minDomain) / (maxDomain - minDomain) * res);
    }
    int prev_x = -1;
    int prev_y = -1;
    ...
    public void add(Object k) {
        ...
        if (x == prev_x) {
            ...
            x_intervals = FindChildIntervals(x + 1, 1, maxRes);
        ...
        if (y == prev_y) {
            ...
            y_intervals = FindChildIntervals(y + 1, 1, maxRes);
    ...
    private int[][] FindChildIntervals(int target, int int_start, int int_end) {
        int start = int_start;
        int end = int_end;
...
//        String coord = node.get(this.valueIndex).asText();
//        String[] coords = coord.split(";");
//        int[][] coordInt = new int[coords.length][2];
//        for (int i = 0; i < coords.length; i++) {
//            String[] xy = coords[i].split(":");
//            coordInt[i][0] = Integer.parseInt(xy[0]);
//            coordInt[i][1] = Integer.parseInt(xy[1]);
//            if (coordInt[i][0] < 0 || coordInt[i][0] >= sketches.length || coordInt[i][1] < 0 || coordInt[i][1] >= sketches[0].length) {
//                throw new IllegalArgumentException("Invalid coordinates for SpatialSketch: (" + coordInt[i][0] + ", " + coordInt[i][1] + ")");
//            }
//            estimate += Double.parseDouble((String) sketches[coordInt[i][0]][coordInt[i][1]].estimate(k));
//        }
//        return estimate;
    }
...
//            coordInt[i][0] = Integer.parseInt(xy[0]);
//            coordInt[i][1] = Integer.parseInt(xy[1]);
//            if (coordInt[i][0] < minX || coordInt[i][0] >= maxX || coordInt[i][1] < minY || coordInt[i][1] >= maxY) {
//                throw new IllegalArgumentException("Invalid coordinates for SpatialSketch: (" + coordInt[i][0] + ", " + coordInt[i][1] + ")");
//            }
//            Estimation est_i = sketches[getNormX(coordInt[i][0])][getNormY(coordInt[i][1])].estimate(rq);
//            estimate += Double.parseDouble((String) est_i.getEstimation());
//        }
        ...
        Estimation est = new Estimation(rq, Double.toString(sum.sum), rq.getUID() + "_" + Arrays.toString(parameters));
        return est;
    }
    ...
    private boolean QueryDyadicInterval(Dyadic2D di, Request rq, Sum sum) {
        ...
        if (sketch.containsKey(key)) {
            ...
            if (sketchArray[x_cell][y_cell] != null) {
               ...
               return true; // Grid exists, but cell is empty. Estimate at this dyadic interval is 0.
            }
        } else {
    ...
    private ArrayList<Dyadic2D> GetDyadicIntervals(int x1, int x2, int y1, int y2) {
        ...
        for (Dyadic1D interval : top_level_intervals) {
            ...
            if (overlapx == 1) {
                ...
                x_intervals = ObtainIntervals(new Dyadic1D(x1 + 1, x2 + 1, 1), interval);
                break;
            } else if (overlapx == 3) {
                // Lower contained. Upper part is out of range, therefore only lower part is required.
                ArrayList<Dyadic1D> sub_intervals = ObtainIntervals(new Dyadic1D(x1 + 1, x2 + 1, 1), interval);
                x_intervals.addAll(sub_intervals);
                break;
            } else if (overlapx == 4) {
                // Upper overlap
                ArrayList<Dyadic1D> sub_intervals = ObtainIntervals(new Dyadic1D(x1 + 1, x2 + 1, 1), interval);
        ...
        for (Dyadic1D interval: top_level_intervals) {
            ...
            if (overlapy == 1) {
                ...
                // Interval is fully contained within the query interval. Therefore other top level intervals do not have to be considered.
                y_intervals = ObtainIntervals(new Dyadic1D(y1 + 1, y2 + 1, 1), interval);
                break;
            } else if (overlapy == 3) {
                // Lower contained. Upper part is out of range, therefore only lower part is required.
                ArrayList<Dyadic1D> sub_intervals = ObtainIntervals(new Dyadic1D(y1 + 1, y2 + 1, 1), interval);
                y_intervals.addAll(sub_intervals);
                break;
            } else if (overlapy == 4) {
                // Upper overlap
                ArrayList<Dyadic1D> sub_intervals = ObtainIntervals(new Dyadic1D(y1 + 1, y2 + 1, 1), interval);
    ...
    private ArrayList<Dyadic1D> ObtainIntervals(Dyadic1D target, Dyadic1D base) {
        ...
        if (base.end - base.start + 1 < resolution) {
            ...
            if (IntervalOverlap(target.start, target.end, lower_base.start, lower_base.end) != 0) {
                lower_intervals = ObtainIntervals(new Dyadic1D(Math.max(target.start, lower_base.start), Math.min(target.end, lower_base.end), target.coverage), lower_base);
                if (lower_intervals.size() == 0) {
                    Dyadic1D partial = base;
                    partial.coverage = (double) (target.end - target.start + 1) / (double) (base.end - base.start + 1);
                    lower_intervals.add(partial);
                }
            }
            if (IntervalOverlap(target.start, target.end, upper_base.start, upper_base.end) != 0) {
                upper_intervals = ObtainIntervals(new Dyadic1D(Math.max(target.start, upper_base.start), Math.min(target.end, upper_base.end), target.coverage), upper_base);
                if (upper_intervals.size() == 0) {
                    Dyadic1D partial = base;
                    partial.coverage = (double) (target.end - target.start + 1) / (double) (base.end - base.start + 1);
                    upper_intervals.add(partial);
                }
            }
